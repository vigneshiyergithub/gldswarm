		...
		for (int i=0; i < num_nodes; i++) 
		{
                    ...
                    try{
                        for(int k = 0; k < tld.length; k++){
                            if(tld[k].length > 0){
                                drs = tld[k][1].getTL().getNode().getAllLanes();
                                break;
                            }
                        }
                        System.out.printf("NumLanes: %d \n", tld[0].length);

                        System.out.printf("Drivelane: %d \t Node comes from: %d \t Node leads to: %d \n", drs[1].getId(), drs[1].getNodeComesFrom().getId(), drs[1].getNodeLeadsTo().getId());
			...
			for(int j=0; j < num_lanes; j++)
                        {
                                ...
                                dr = tld[i][j].getTL().getNode().getInboundLanes()[0];
                                drs = tld[i][j].getTL().getNode().getLanesLeadingFrom(dr, RoaduserFactory.CAR);
                                //System.out.printf("Id: %d \n",tld[i][j].getTL().getNode().getId());
                                System.out.printf("Id: %d \n",dr.getId());

                                 for(int k=0;k<drs.length;k++){
                                    System.out.printf("\t Id: %d \n", drs[k].getId());
                                }
                                //if(tld[i][j].getTL().getNode() instanceof Junction)System.out.printf("Id = %d \n", tld[i][j].getTL().getNode().getId());
                        }
